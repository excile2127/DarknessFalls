...
public class PlayerDashing : PlayerAction
{
    ...
    public float torchlighOnDoubleDash;
        ...
        if (Input.GetKeyDown(KeyCode.LeftShift) && _actionController.Grounded && Input.GetAxis("Horizontal") != 0)
        {
            ...
            switch (_dashState)
            {
                case DashState.Ready:
                    ...
                    _body.velocity = new Vector2(Input.GetAxis("Horizontal") * dashSpeed, _body.velocity.y);
                ...
                case DashState.Dashing: case DashState.DashCooldown:
                    if (!_actionController.DeathsDoor)
                    {
                        ...
                        Messenger<float>.Broadcast(GameEvent.TORCHLIGHT_UPDATE, torchlighOnDoubleDash);
                        // Set the hurtbox to inactive
                        _hurtbox.active = false;
    ...
    public override void AfterActionUpdate()
    {
        ...
        switch (_dashState)
        {
            case DashState.Dashing:
                ...
                _body.velocity = new Vector2(Math.Sign(_body.velocity.x) * dashSpeed, _body.velocity.y);
